"""Tail-first variant of the parallel Eppstein counter.

Identical to :func:`parallel_processing.eppstein_parallel.profile_eppstein_parallel`
except for batch construction: the outer-vertex indices are dispatched in
*reverse* degeneracy order. The heavy vertices cluster at the tail of the
ordering (measured in artifacts/workload-*.csv), so the block strategy hands
them out last and leaves early-finishing workers idle; dispatching the tail
first lets imap_unordered fill the gaps with the cheap head batches.
"""
...
def profile_eppstein_parallel(
    ...
    indices = list(range(len(ordering)))[::-1]
    batches = [
