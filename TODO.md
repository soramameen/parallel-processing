...
- [x] 早期終了（候補集合がクリークなら直接答える）を試作して測る（`eppstein_et.py`: ca-HepPh 1.24 倍、ca-AstroPh 1.08 倍、合成グラフで最大 11%、soc で 7% の損。シミュでは ca-HepPh の並列上限が 7.48 倍から 32 コアで 31.8 倍に）
- [ ] 早期終了の損を消す（小さい P では判定しない等）、並列版にも入れて ca-HepPh の並列スケーリングを測る
